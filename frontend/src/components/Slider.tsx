...
const Slider = ({ data, sliderId }: any) => {
  ...
  return (
    ...
    >
      ...
      <div
        id={sliderId}
        className="flex flex-nowrap gap-4 overflow-x-scroll scroll-smooth p-4 scrollbar-none w-full"
      >
        {data
          ? data.map((item: any, index: number) => (
              <div
                key={index}
                className="relative flex w-64 flex-shrink-0 flex-col rounded-md transition-transform duration-200 ease-in-out hover:scale-105"
              >
                <div className=" h-96">
