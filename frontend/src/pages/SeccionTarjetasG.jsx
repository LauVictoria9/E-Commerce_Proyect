...
import TG from "../img/Componentes.jpg";
import Slider from "../components/Slider";
import { obtenerProductosTipo } from "../api/productos.api";

export default function SeccionTG() {
  const [componentesTG, setComponentesTG] = useState([]);
  ...
  useEffect(() => {
    ...
    const obtenerProductos = async () => {
      ...
      const response = await obtenerProductosTipo("grafica", 8);
      ...
      while (flag) {
        ...
        const response = await obtenerProductosTipo("grafica", 8, index);
        const componentesPaginaActual = response.data;
        productosPorPagina.push(componentesPaginaActual.results);
      ...
      setComponentesTG(productosPorPagina);
    };
  ...
  return componentesTG ? (
    <main className="pt-8 pb-16 ">
      ...
      <section className="">
        ...
        {componentesTG.map((componentesTG, index) => (
          <>
            {console.log(componentesTG.id)}
            <div key={index} className="flex">
              ...
              >
                ...
                <Slider sliderId={index} data={componentesTG} />
              </div>
            </div>
            <div className="h-16" />
          </>
