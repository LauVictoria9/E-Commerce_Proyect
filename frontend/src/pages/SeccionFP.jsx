...
import Asus from "../img/FP.png";
import FP from "../img/Componentes.jpg";
import Slider from "../components/Slider";
import { obtenerProductosTipo } from "../api/productos.api";

export default function SeccionFP() {
  const [componentesFP, setcomponentesFP] = useState([]);
  ...
  useEffect(() => {
    ...
    const obtenerProductos = async () => {
      ...
      const response = await obtenerProductosTipo("fuente de poder", 8);
      ...
      while (flag) {
        ...
        const response = await obtenerProductosTipo(
          "fuente de poder",
          8,
          index
        );
      ...
      setcomponentesFP(productosPorPagina);
    };
  ...
  return componentesFP ? (
    <main className="pt-8 pb-16 ">
      ...
      <section className="">
        ...
        {componentesFP.map((componentesFP, index) => (
          <>
            {console.log(componentesFP.id)}
            <div key={index} className="flex">
              ...
              >
                ...
                <Slider sliderId={index} data={componentesFP} />
              </div>
            </div>
            <div className="h-16" />
          </>
